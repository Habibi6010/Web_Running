...
class Analysis_Landmarks:
    ...
    def mediapipe_feature_selection(landmarks, width, height, scale_factor):
        # Extract the coordinates of the landmarks
        pose_landmarks = [
            [int((lmk[0])*width), int((lmk[1])*height), round(lmk[2], 3), round(lmk[3], 3)] for lmk in landmarks]
        landmarks_features = {}
        landmarks_features['hip_right'] = pose_landmarks[24]
        landmarks_features['hip_left'] = pose_landmarks[23]
        landmarks_features['knee_right'] = pose_landmarks[26]
        landmarks_features['knee_left'] = pose_landmarks[25]
        landmarks_features['ankle_right'] = pose_landmarks[28]
        landmarks_features['ankle_left'] = pose_landmarks[27]
        landmarks_features['heel_right'] = pose_landmarks[30]
        landmarks_features['heel_left'] = pose_landmarks[29]
        landmarks_features['toe_right'] = pose_landmarks[32]
        landmarks_features['toe_left'] = pose_landmarks[31]
        landmarks_features['ear_right'] = pose_landmarks[8]
        landmarks_features['ear_left'] = pose_landmarks[7]
        landmarks_features['shoulder_right'] = pose_landmarks[12]
        landmarks_features['shoulder_left'] = pose_landmarks[11]
        landmarks_features['elbow_right'] = pose_landmarks[14]
        landmarks_features['elbow_left'] = pose_landmarks[13]
        landmarks_features['wrist_right'] = pose_landmarks[16]
        landmarks_features['wrist_left'] = pose_landmarks[15]
        # Calculate the features toe off
        landmarks_features['angle_right_hip_knee_ankle'] = int(tools.calculate_angle(
            landmarks_features['hip_right'], landmarks_features['knee_right'], landmarks_features['ankle_right']))
        landmarks_features['angle_left_hip_knee_ankle'] = int(tools.calculate_angle(
            landmarks_features['hip_left'], landmarks_features['knee_left'], landmarks_features['ankle_left']))
        landmarks_features['distance_right_hip_ankle'] = round(tools.calculate_distance_yolo(
            landmarks_features['hip_right'], landmarks_features['ankle_right'])*scale_factor, 2)
        landmarks_features['distance_left_hip_ankle'] = round(tools.calculate_distance_yolo(
            landmarks_features['hip_left'], landmarks_features['ankle_left'])*scale_factor, 2)
        landmarks_features['angle_right_toe_heel_x'] = int(tools.calculate_angle_with_x_axis(
            landmarks_features['toe_right'], landmarks_features['heel_right']))
        landmarks_features['angle_left_toe_heel_x'] = int(tools.calculate_angle_with_x_axis(
            landmarks_features['toe_left'], landmarks_features['heel_left']))
        # Calculate the features full-flight
        if pose_landmarks[24][3] > 0.5 and pose_landmarks[23][3] > 0.5:
            landmarks_features['hip_mid'] = [int((pose_landmarks[24][0]+pose_landmarks[23][0])/2), int(
                (pose_landmarks[24][1]+pose_landmarks[23][1])/2)]
        elif pose_landmarks[24][3] > 0.5:
            landmarks_features['hip_mid'] = [
                pose_landmarks[24][0], pose_landmarks[24][1]]
        elif pose_landmarks[23][3] > 0.5:
            landmarks_features['hip_mid'] = [
                pose_landmarks[23][0], pose_landmarks[23][1]]
        if pose_landmarks[7][3] > 0.5 and pose_landmarks[8][3] > 0.5:
            landmarks_features['ear_mid'] = [int((pose_landmarks[7][0]+pose_landmarks[8][0])/2), int(
                (pose_landmarks[7][1]+pose_landmarks[8][1])/2)]
        elif pose_landmarks[7][3] > 0.5:
            landmarks_features['ear_mid'] = [
                pose_landmarks[7][0], pose_landmarks[7][1]]
        elif pose_landmarks[8][3] > 0.5:
            landmarks_features['ear_mid'] = [
                pose_landmarks[8][0], pose_landmarks[8][1]]
        landmarks_features['distance_knee'] = round(tools.calculate_distance_yolo(
            pose_landmarks[26], pose_landmarks[25])*scale_factor, 2)
        # Calculate the features touh-down
        landmarks_features['angle_right_shoulder_elbow_wrist'] = int(tools.calculate_angle(
            landmarks_features['shoulder_right'], landmarks_features['elbow_right'], landmarks_features['wrist_right']))
        landmarks_features['angle_left_shoulder_elbow_wrist'] = int(tools.calculate_angle(
            landmarks_features['shoulder_left'], landmarks_features['elbow_left'], landmarks_features['wrist_left']))
        # Calculate the features full-support
        landmarks_features['distance_knee_together'] = round(tools.calculate_distance_yolo(
            landmarks_features['knee_right'], landmarks_features['knee_left'])*scale_factor,2)
        return landmarks_features
    
    def yolo_feature_selection(landmarks, scale_factor):
        landmarks_features = {}
        landmarks_features['hip_right'] = [
            int(landmarks[12][0]), int(landmarks[12][1])]
        landmarks_features['hip_left'] = [
            int(landmarks[11][0]), int(landmarks[11][1])]
        landmarks_features['knee_right'] = [
            int(landmarks[14][0]), int(landmarks[14][1])]
        landmarks_features['knee_left'] = [
            int(landmarks[13][0]), int(landmarks[13][1])]
        landmarks_features['ankle_right'] = [
            int(landmarks[16][0]), int(landmarks[16][1])]
        landmarks_features['ankle_left'] = [
            int(landmarks[15][0]), int(landmarks[15][1])]

        landmarks_features['ear_right'] = [int(landmarks[4][0]), int(landmarks[4][1])]
        landmarks_features['ear_left'] = [int(landmarks[3][0]), int(landmarks[3][1])]

        landmarks_features['shoulder_right'] = [
            int(landmarks[6][0]), int(landmarks[6][1])]
        landmarks_features['shoulder_left'] = [
            int(landmarks[5][0]), int(landmarks[5][1])]
        landmarks_features['elbow_right'] = [
            int(landmarks[8][0]), int(landmarks[8][1])]
        landmarks_features['elbow_left'] = [
            int(landmarks[7][0]), int(landmarks[7][1])]
        landmarks_features['wrist_right'] = [
            int(landmarks[10][0]), int(landmarks[10][1])]
        landmarks_features['wrist_left'] = [
            int(landmarks[9][0]), int(landmarks[9][1])]
        # Calculate the features toe off
        landmarks_features['angle_right_hip_knee_ankle'] = int(tools.calculate_angle(
            landmarks_features['hip_right'], landmarks_features['knee_right'], landmarks_features['ankle_right']))
        ...
        # Calculate the features full-flight
        if landmarks[12][2] > 0.5 and landmarks[11][2] > 0.5:
            landmarks_features['hip_mid'] = [int(
                (landmarks[12][0]+landmarks[11][0])/2), int((landmarks[12][1]+landmarks[11][1])/2)]
        elif landmarks[12][2] > 0.5:
            landmarks_features['hip_mid'] = landmarks_features['hip_right']
        elif landmarks[11][2] > 0.5:
            landmarks_features['hip_mid'] = landmarks_features['hip_left']
        if landmarks[3][2] > 0.5 and landmarks[4][2] > 0.5:
            landmarks_features['ear_mid'] = [int(
                (landmarks[3][0]+landmarks[4][0])/2), int((landmarks[3][1]+landmarks[4][1])/2)]
        elif landmarks[3][2] > 0.5:
            landmarks_features['ear_mid'] = landmarks_features['ear_left']
        elif landmarks[4][2] > 0.5:
            landmarks_features['ear_mid'] = landmarks_features['ear_right']
        landmarks_features['distance_knee'] = round(tools.calculate_distance_yolo(
            landmarks_features['knee_right'], landmarks_features['knee_left'])*scale_factor, 2)
        # Calculate the features touh-down
        landmarks_features['angle_right_shoulder_elbow_wrist'] = int(tools.calculate_angle(
        ...
        landmarks_features['angle_left_shoulder_elbow_wrist'] = int(tools.calculate_angle(
            landmarks_features['shoulder_left'], landmarks_features['elbow_left'], landmarks_features['wrist_left']))
        # Calculate the features full-support
        landmarks_features['distance_knees_together'] = round(tools.calculate_distance_yolo(
            landmarks_features['knee_right'], landmarks_features['knee_left'])*scale_factor,2)
        
        return landmarks_features
    
    def mediapipe_features_plot_2d(data_list):
