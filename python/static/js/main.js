...
* Template URL: http://bootstrapmade.com/free-html-bootstrap-template-my-resume/
  ...
  fetch('/login', {
    ...
    .then(data => {
      if (data.accsess) {
        ...
        window.location.href = `/dashboard?username=${encodeURIComponent(username)}&useremail=${encodeURIComponent(useremail)}`;
...
function previewVideo() {
  ...
  if (fileInput.files && fileInput.files[0]) {
    ...
    fetch('/convert_upload_video_preview', {
      method: 'POST',
      body: fd
    })
      .then(response => response.json())
...
function handelVideLogButton(event) {
  ...
  window.location.href = `/videolog?useremail=${encodeURIComponent(useremail)}&username=${encodeURIComponent(username)}`;
}
...
function handelDashboardButton (){
  ...
  console.log(username);
  window.location.href = `/dashboard?useremail=${encodeURIComponent(useremail)}&username=${encodeURIComponent(username)}`;
...
function getRunneerInfoSaveSessionStorage(userEmail) {
  ...
  fetch('/find_runner_info',{method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({userEmail: userEmail })
  })
  .then(response => response.json())
...
function handelrankLogButton(){
  ...
  console.log(username);
  window.location.href = `/rankinglog?useremail=${encodeURIComponent(useremail)}&username=${encodeURIComponent(username)}`;
}
...
function handleCompareScoreAnalysis(){
  ...
  fetch('/compare_scores_same_season_category_event_gender', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(dataToSend)
  })
